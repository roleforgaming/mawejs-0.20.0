...
function updateDocTitle(updateDoc, value) {
  updateDoc((doc) => {
    doc.head.title = value;
  });
}
function updateDocSubtitle(updateDoc, value) {
  updateDoc((doc) => {
    doc.head.subtitle = value;
  });
}
function updateDocAuthor(updateDoc, value) {
  updateDoc((doc) => {
    doc.head.author = value;
  });
}
function updateDocPseudonym(updateDoc, value) {
  updateDoc((doc) => {
    doc.head.pseudonym = value;
  });
}
          ...
          <AccordionDetails>
            <VBox>
              <TextField
                label="Name"
                value={head.name ?? ""}
                onChange={(e) => updateDocName(updateDoc, e.target.value)}
              />
              <TextField
                label="Title"
                value={head.title ?? ""}
                onChange={(e) => updateDocTitle(updateDoc, e.target.value)}
              />
              <TextField
                label="Subtitle"
                value={head.subtitle ?? ""}
                onChange={(e) => updateDocSubtitle(updateDoc, e.target.value)}
              />
            </VBox>
          </AccordionDetails>
        </Accordion>
          ...
          <AccordionDetails>
            <VBox>
              <TextField
                label="Author"
                value={head.author ?? ""}
                onChange={(e) => updateDocAuthor(updateDoc, e.target.value)}
              />
              <TextField
                label="Pseudonym"
                value={head.pseudonym ?? ""}
                onChange={(e) => updateDocPseudonym(updateDoc, e.target.value)}
              />
            </VBox>
          </AccordionDetails>
        </Accordion>
...
class EditHeadButton extends React.PureComponent {
  render() {
    ...
    return (
      <PopupState variant="popover" popupId="head-edit">
        {(popupState) => (
          <React.Fragment>
            <Button
...
export class OpenFolderButton extends React.PureComponent {
  render() {
    ...
    return (
      <IconButton
        tooltip="Open Folder"
        onClick={(e) => cmdOpenFolder(filename)}
      >
        <Icon.Action.Folder />
      </IconButton>
    );
          ...
          <React.Fragment>
            ...
            <Menu {...bindMenu(popupState)}>
              {choices.map((type, index) =>
                this.menuItem(popupState, index, type, setSelected),
              )}
            </Menu>
          </React.Fragment>
        )}
...
export class FormatWords extends React.PureComponent {
  render() {
    ...
    switch (format) {
      case "numbers":
        return this.number(target, text, missing, padding);
      case "compact":
        return this.compact(target, text, missing, padding);
      case "percent":
        return this.percent(cumulative, total, text, missing);
      case "cumulative":
        return this.cumulative(cumulative, total, text, missing);
      default:
        break;
    }
    return null;
    ...
    return (
      <>
        {missing ? (
          ...
          <Icon.Starred
            sx={{ ...style, color: "#59F", marginRight: "4pt", fontSize: 14 }}
          />
        )}
