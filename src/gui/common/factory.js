...
export class Filler extends React.PureComponent {
  render() {
    ...
    return (
      <div style={{ display: "flex", flexGrow: weight, ...style }} {...props} />
    );
  }
}
...
export class MakeToggleGroup extends React.PureComponent {
  render() {
    const { buttons, choices, disabled, selected, setSelected, ...props } =
      this.props;
    ...
    return (
      ...
      >
        {choices.map((choice) =>
          this.constructor.makeButton(buttons, disabled, choice),
        )}
      </ToggleButtonGroup>
    );
  }
  ...
  static makeButton(buttons, disabled, choice) {
    ...
    if (!(choice in buttons))
      return (
        <ToggleButton key={choice} disabled={isDisabled} value={choice}>
          {choice}
        </ToggleButton>
      );
...
export function Loading({ className, style }) {
  return (
    <Filler className={className} style={style}>
...
export { MuiMenu as Menu, MuiMenuItem };
//Menu, MenuItem, MenuList,
...
export function MenuItem({
  ...
  return (
    <MuiMenuItem value={value} disabled={disabled} onClick={onClick}>
      {startAdornment ? <ListItemIcon>{startAdornment}</ListItemIcon> : null}
      {title ? <ListItemText>{title}</ListItemText> : null}
      {endAdornment ? (
        <Typography sx={{ color: "text.secondary" }}>{endAdornment}</Typography>
      ) : null}
    </MuiMenuItem>
  );
}
